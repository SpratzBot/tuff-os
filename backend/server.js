...
function userIsMember(conversationId, userId) {
  return Boolean(db.prepare('SELECT 1 FROM conversation_members WHERE conversation_id = ? AND user_id = ?').get(conversationId, userId));
}

app.get('/api/health', (_req, res) => res.json({ ok: true, service: 'tuffos-api', version: '0.2.1' }));
...
// Keep the frontend and API in one deployable unit. On Vercel this Express
// app is mounted by /api/index.js; locally it still runs as a normal server.
